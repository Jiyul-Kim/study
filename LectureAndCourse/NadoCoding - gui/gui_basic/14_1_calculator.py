from tkinter import *
import tkinter.font

root = Tk()
root.title("나도 GUI")
root. geometry("640x480") # 가로 x 세로 + x 좌표 + y좌표

font= tkinter.font.Font(size=6)

# 01
btn_f16 = Button(root,text="F16", anchor='s',width=5, height=4,font=font)
btn_f17 = Button(root,text="F17", anchor='s',width=5, height=4,font=font)
btn_f18 = Button(root,text="F18", anchor='s',width=5, height=4,font=font)
btn_f19 = Button(root,text="F19", anchor='s',width=5, height=4,font=font)

btn_f16.grid(row=0, column=0, sticky=N+E+W+S, padx=2, pady=2)
btn_f17.grid(row=0, column=1, sticky=N+E+W+S, padx=2, pady=2)
btn_f18.grid(row=0, column=2, sticky=N+E+W+S, padx=2, pady=2)
btn_f19.grid(row=0, column=3, sticky=N+E+W+S, padx=2, pady=2)

# 02
btn_clear = Button(root,text="clear", width=5, height=2)
btn_equal = Button(root,text="=", width=5, height=2)
btn_slash = Button(root,text="/", width=5, height=2)
btn_asterisk = Button(root,text="*", width=5, height=2)

btn_clear.grid(row=1, column=0, sticky=N+E+W+S, padx=2, pady=2)
btn_equal.grid(row=1, column=1, sticky=N+E+W+S, padx=2, pady=2)
btn_slash.grid(row=1, column=2, sticky=N+E+W+S, padx=2, pady=2)
btn_asterisk.grid(row=1, column=3, sticky=N+E+W+S, padx=2, pady=2)

# 03
btn_7 = Button(root,text="7", width=5, height=2)
btn_8 = Button(root,text="8", width=5, height=2)
btn_9 = Button(root,text="9", width=5, height=2)
btn_hyphen = Button(root,text="-", width=5, height=2)

btn_7.grid(row=2, column=0, sticky=N+E+W+S, padx=2, pady=2)
btn_8.grid(row=2, column=1, sticky=N+E+W+S, padx=2, pady=2)
btn_9.grid(row=2, column=2, sticky=N+E+W+S, padx=2, pady=2)
btn_hyphen.grid(row=2, column=3, sticky=N+E+W+S, padx=2, pady=2)

# 04
btn_4 = Button(root,text="4", width=5, height=2)
btn_5 = Button(root,text="5", width=5, height=2)
btn_6 = Button(root,text="6", width=5, height=2)
btn_plus = Button(root,text="+", width=5, height=2)

btn_4.grid(row=3, column=0, sticky=N+E+W+S, padx=2, pady=2)
btn_5.grid(row=3, column=1, sticky=N+E+W+S, padx=2, pady=2)
btn_6.grid(row=3, column=2, sticky=N+E+W+S, padx=2, pady=2)
btn_plus.grid(row=3, column=3, sticky=N+E+W+S, padx=2, pady=2)

# 05
btn_1 = Button(root,text="1", width=5, height=2)
btn_2 = Button(root,text="2", width=5, height=2)
btn_3 = Button(root,text="3", width=5, height=2)
btn_enter = Button(root,text="\n\n\n\nenter", width=5, height=2)


btn_1.grid(row=4, column=0, sticky=N+E+W+S, padx=2, pady=2)
btn_2.grid(row=4, column=1, sticky=N+E+W+S, padx=2, pady=2)
btn_3.grid(row=4, column=2, sticky=N+E+W+S, padx=2, pady=2)
btn_enter.grid(row=4, column=3, rowspan=2, sticky=N+E+W+S, padx=2, pady=2) # 현재 위치로부터 아래쪽으로 더함



# 06
btn_0 = Button(root,text="0", width=5, height=2)
btn_period = Button(root,text=".", width=5, height=2)

btn_0.grid(row=5, column=0, columnspan=2, sticky=N+E+W+S, padx=2, pady=2)
btn_period.grid(row=5, column=2, sticky=N+E+W+S, padx=2, pady=2)



root.mainloop()