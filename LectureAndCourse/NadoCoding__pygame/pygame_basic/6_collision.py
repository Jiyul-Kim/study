...
# 적 캐릭터
enermy=pygame.image.load('E:/PYTHONWORKSPACE/pygame_basic/enermy.png')
enermy_size = enermy.get_rect().size # 이미지의 크기를 구해옴
enermy_width = enermy_size[0] # 캐릭터의 가로 크기
enermy_height = enermy_size [1] # 캐릭터의 세로 크기
enermy_x_pos = (screen_width / 2) - (enermy_width / 2) # 화면 가로의 절반 크기에 해당하는 곳에 위치 (가로)
enermy_y_pos = (screen_height / 2) - (enermy_height / 2) # 화면 세로 크기 가장 아래에 해당하는 곳에 위치 (세로)
    ...
    for event in pygame.event.get(): # 어떤 이벤트가 발생하였는가?
        ...
        if event.type == pygame.KEYDOWN: # 키가 눌러졌는지 확인
            if event.key == pygame.K_LEFT: # 캐릭터를 왼쪽으로
                ...
                to_y += character_speed
        if event.type == pygame.KEYUP: # 방향키를 멈춤
